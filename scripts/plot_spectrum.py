...
ux_coef = np.abs(temp_ux['c'])
uy_coef = np.abs(temp_uy['c'])
...
n_count,bins = np.histogram(k.flatten(),bins=5*params.N)

n, bins = np.histogram(k.flatten(),bins=5*params.N,weights=ps.flatten())

# Remove data points with power below threshold. This arises from plotting issues and is to be fixed
