...
def cross_pspec(k,field_1,field_2):
    numerator = np.conj(field_1['c'])*field_2['c']
    denominator = np.sqrt(np.conj(field_1['c'])*field_1['c']*np.conj(field_2['c'])*field_2['c'])

    cross_pspec = numerator/denominator
    n_count,bins = np.histogram(k.flatten(),bins=2*N_filter)

    n, bins = np.histogram(k.flatten(),bins=2*N_filter,weights=cross_pspec.flatten())
...
for i in [0,1]:
    for j in [1,0]:
        ...
        bins, cross_ps = cross_pspec(k,pred,true)

        ax[l].plot(bins[:-1],cross_ps)
        ax[l].set_title(titles[l],fontsize=26)

        ax[l].set_ylabel("Normalized Cross" + "$P(k)$",fontsize=20)
        ...
        l += 1
plt.savefig('cross_ps.png',bbox_inches='tight')
